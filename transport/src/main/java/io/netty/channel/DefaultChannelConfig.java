...
public class DefaultChannelConfig implements ChannelConfig {
    ...
    private volatile int writeSpinCount = 16;//自旋写的次数
    @SuppressWarnings("FieldMayBeFinal")
    private volatile int autoRead = 1;//是否自动读
    private volatile boolean autoClose = true;
    private volatile WriteBufferWaterMark writeBufferWaterMark = WriteBufferWaterMark.DEFAULT;//写水位
    private volatile boolean pinEventExecutor = true;

    public DefaultChannelConfig(Channel channel) {
    	//创建一个AdaptiveRecvByteBufAllocator，可调节的接收缓冲区
