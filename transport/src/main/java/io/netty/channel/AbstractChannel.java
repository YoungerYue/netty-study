...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
     ...
     */
    //基本抽象实现
	//基本的属性，要进行数据的读写，需要有接收缓冲区，所以有了recvHandle处理器
	//写出去的时候需要有写缓冲区ChannelOutboundBuffer ，注意ChannelOutboundBuffer是初始化的时候就会创建，就创建一次
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
			...
			//只能当前线程是eventLoop的线程才可以注册，防止多线程并发问题，所以即使多线程来操作，也是安全的，
			// 会按照一定顺序提交到任务队列里
        ...
        //注册过程要做的事，进行真正的注册逻辑doRegister，
		// 其实就是将NIO通道注册到Selector上，然后进行处理器的待添加事件的处理，注册回调成功，管道传递注册事件，
		// 如果是第一次注册，管道传递通道激活事件，否则是设置自动读的话就注册读监听
        ...
        //做具体的doBind，如果通道开始没激活，绑定后激活的话，就开一个延时的任务，进行激活事件传递，最后回调绑定成功。
        @Override
        ...
        private void close(final ChannelPromise promise, final Throwable cause,
            ...
            if (closeExecutor != null) {
                closeExecutor.execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                            // Execute the close.关闭通道，具体的关闭逻辑和回调，具体逻辑是在通道中实现的
                            doClose0(promise);
                        } finally {
        ...
        //write操作最后就是写入出站缓冲区。如果出站缓冲区关闭了，那就无用写了，释放消息即可，否则就封装后放入出站缓冲区里，里面是个单链表
        @Override
