...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    AbstractChannelHandlerContext(DefaultChannelPipeline pipeline, EventExecutor executor,
        ...
        this.pipeline = pipeline;//保存管道
        this.executor = executor;//保存执行器
        this.executionMask = mask(handlerClass);//创建执行事件遮罩
    ...
    //可以看到，执行器不一定是通道的，如果创建的时候没传，那就用通道的，否则可能是其他通道的执行器
    @Override
    ...
    public ChannelPromise newPromise() {
    	//将当前的通道和事件循环执行器传进去，这样就能保证回调的时候我们就能拿到通道实例
		//这里为什么还要有个执行器，通道里不是有么，其实这里的执行器是处理器上下文的执行器，可能和通道的不一样，看你创建的时候有没传
