...
public abstract class AbstractNioByteChannel extends AbstractNioChannel {
    ...
    protected class NioByteUnsafe extends AbstractNioUnsafe {
    	...
    	//这里可能要涉及一个半关闭的概念，简单点就是说我们客户端和服务端的socket其实都有两个缓冲区，即读缓冲区和写缓冲区，
		// 如果TCP要分手了，客户端就会先发送关闭请求，然后把写缓冲区关闭了，就不写了，但是这个时候他是可以读客户端的消息的，
		// 即读缓冲区还没关，所以叫做半关闭。
		// 所以这里就是判断是否是把读缓冲区关了，如果关了，就直接传递自定义消息，
		// 否则就判断是否配置了半关闭，是的话就进行读关闭，传递自定义消息，否则就关闭通道了。
        ...
        //处理读异常
		//如果出现了读的异常，因为有缓冲区申请在那边，
		// 如果缓冲区有数据，那就先把数据传递下去，否则就直接释放，然后还是按普通逻辑去统计，传递读完成事件，最后传递异常事件
        ...
        //读客户端数据
		//其实逻辑和读客户端连接差不多，只不过这次是需要接收缓冲区分配器分配缓冲区来接受读的信息。
		// 然后接受一次传递一次读事件。这里为什么要读一次传一次，而不是全部读完再传呢，
		// 我想可能是因为这样读一部分处理一分部分，处理完了就释放内存，可以提高吞吐量，而且也不至于数据有积压，可能造成内存溢出。
        @Override
        public final void read() {
            final ChannelConfig config = config();// channel的配置，前面涉及到了
            ...
            try {
                do {
                    ...
                    readPending = false;
                    pipeline.fireChannelRead(byteBuf);// handler链表开始执行channelRead方法。 //管道里传递消息
                    byteBuf = null;
                } while (allocHandle.continueReading());

                allocHandle.readComplete();
                pipeline.fireChannelReadComplete();// handler链表开始执行channelReadComplete方法。 //读取完成
