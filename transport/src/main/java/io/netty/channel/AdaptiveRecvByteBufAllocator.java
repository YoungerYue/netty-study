 ...
 */
//可调节的缓冲区分配器，这个是有限制的，默认最小尺寸64，最大是65536，初始是1024，
// 初始化时候会创建一个数组，里面有所有可分配的尺寸，从16到2^30。
// 16-496是间隔16分的，512以上是2倍分的。然后会将最小最大初始尺寸都转化为索引，方便操作
public class AdaptiveRecvByteBufAllocator extends DefaultMaxMessagesRecvByteBufAllocator {
	...
	/**
	 * 真正进行尺寸伸缩的处理器
	 * 每次设置上一次读取的字节数时，会判断真实读取的字节数是不是把分配的缓冲区给填满了，如果满了就要进行缓冲区尺寸的伸缩。
	 * 伸缩算法就是如果真实读取的字节数小于等于当前尺寸的前一个尺寸大小，且要连续两次，那就会把空间缩小成前一个尺寸大小。
	 * 如果真实读取字节数大于等于预测的接收缓冲区大小，那就扩容，每次扩容是当前尺寸的后4个尺寸大小，但是不超过最大尺寸。
	 *
	 * 举个例子16,32,48,64,80,96.6个尺寸，开始是在32，
	 * 如果两次发现真实的读取数据都小于等于16，那就设置成16，
	 * 如果发现数据大于等于32，就跳4个位置，就是96。
	 * 为什么要扩容的时候跳那么多呢，我想可能是因为扩容太小的话会可能会有多次扩容，多次申请直接缓冲区，直接缓冲区的创建和释放是有性能消耗的
	 */
