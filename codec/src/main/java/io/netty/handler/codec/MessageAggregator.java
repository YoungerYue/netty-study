...
public abstract class MessageAggregator<I, S, C extends ByteBufHolder, O extends ByteBufHolder>
    ...
    private final int maxContentLength;//最大消息长度
	 ...
	 * 真正的聚合
	 *
	 * 1.如果是开始消息，也就不是请求体，那就开始判断是否有Except:100-continue头信息，有的话根据长度和是否支持来判断是否要返回响应。
	 * 之后判断如果前面解码失败，就直接整合消息体返回，否则就创建复合缓冲区，如果是消息体的话就添加进去，然后封装成一个完整的消息类型。
	 * 2.如果是消息体了，就加入到复合画冲去里，然后判断是否是最后一个消息体，是的话就进行最后的整合，其实就是设置Content-Length头信息。
	 */
    @Override
