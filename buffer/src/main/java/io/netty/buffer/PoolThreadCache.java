...
final class PoolThreadCache {
    ...
    PoolThreadCache(PoolArena<byte[]> heapArena, PoolArena<ByteBuffer> directArena,
        ...
        if (directArena != null) {//直接缓冲区的缓存
			//创建子页类型的缓存数组
			//tiny类型默认是32个SubPageMemoryRegionCache，每个内部可以缓存512个Entry
            tinySubPageDirectCaches = createSubPageCaches(
                    tinyCacheSize, PoolArena.numTinySubpagePools, SizeClass.Tiny);
			//small类型默认是4个SubPageMemoryRegionCache，每个内部可以缓存256个Entry
            smallSubPageDirectCaches = createSubPageCaches(
                    smallCacheSize, directArena.numSmallSubpagePools, SizeClass.Small);

            numShiftsNormalDirect = log2(directArena.pageSize);//获取2的次数 // 2 ^ 13 = 8192  这里是 13

			// 创建Normal缓存数组 ， normalCacheSize = 64 ， maxCachedBufferCapacity = 32K
    ...
    private static <T> MemoryRegionCache<T>[] createNormalCaches(
        ...
        if (cacheSize > 0 && maxCachedBufferCapacity > 0) {
            ...
            int arraySize = Math.max(1, log2(max / area.pageSize) + 1);//默认是3 // 32K 就是 8 k -> 16k -> 32 k 那么 arraySize = 3
    ...
    boolean add(PoolArena<?> area, PoolChunk chunk, ByteBuffer nioBuffer,
        ...
        return cache.add(chunk, nioBuffer, handle);//  添加到队列中
    }
    ...
    private MemoryRegionCache<?> cacheForSmall(PoolArena<?> area, int normCapacity) {
        ...
        return cache(smallSubPageHeapCaches, idx);// 根据数组下标，获取对应Cache
    }
    ...
    private abstract static class MemoryRegionCache<T> {
        ...
        private final Queue<Entry<T>> queue;//实体队列，一个MpscArrayQueue多生产者单消费者的队列
        private final SizeClass sizeClass;//类型
        private int allocations;//已经释放了多少个，表示缓存已经分配出去了多少个，这个在后面释放的时候会用到
         ...
         */
        //开始尝试添加，会先将要缓存的数据封装成一个实体，然后尝试放进队列里，如果队列满了，放不了，就把实体回收，返回false。
		// 如果能放，就直接返回true。
        @SuppressWarnings("unchecked")
